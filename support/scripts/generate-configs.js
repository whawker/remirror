...
// Added to tsconfig files to show that the tsconfig file was autogenerated and
// should not be edited.
const autoGeneratedFlag = {
  __AUTO_GENERATED__: 'To update run: `pnpm generate:config`',
};
...
async function generatePackageTsConfigs() {
  ...
  async function writePackageTsconfig(pkg) {
    ...
    const tsconfigCompilerOptions = pkg.types
      ? { declaration: true, composite: true, noEmit: false }
      : { noEmit: true, declaration: false };

    // Create the json for the tsconfig which will be written to the tssconfig file.
    const tsconfig = {
      ...basePackageTsconfig,
      extends: relative(pkg.location, baseDir(paths.baseTsconfig)),
      compilerOptions: {
