...
import { MentionAtomExtension, MentionAtomExtensionSuggestCommand, MentionAtomOptions } from '..';
...
describe('constructor', () => {
  it('is created with the correct options', () => {
    ...
    const mentions = new MentionAtomExtension({
      matchers: [matcher],
    });
  ...
  it('can be created with partial matchers', () => {
    const mentionOne = new MentionAtomExtension({
...
function create(options: MentionAtomOptions, handlers: GetHandler<MentionAtomOptions> = {}) {
...
describe('plugin', () => {
  const options: MentionAtomOptions = {
    matchers: [
  ...
  const mocks = {
    onChange: jest.fn(),
    onExit: jest.fn(
      ({ command }: SuggestExitHandlerParameter<MentionAtomExtensionSuggestCommand>) => {
        command({ appendText: '' });
      },
    ),
  };
